...
$ct = $config[ACCOUNT_ARZZUP];
$to = new TwistOAuth($ct['CONSUMER_KEY'], $ct['CONSUMER_SECRET'], $ct['OAUTH_TOKEN'], $ct['OAUTH_TOKEN_SECRET']);
...
if ($fp) {
    ...
    while (!feof($fp)) {
        ...
        $msg = "[{$res['id']}]: {$res['text']}";
        echo 'posted: ' . $msg;
        socket_sendto($sock, $msg, strlen($msg), 0, 'localhost', 10007);
    }
    fclose($fp);
    socket_close($sock);
}
